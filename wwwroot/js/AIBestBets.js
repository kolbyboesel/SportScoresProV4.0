...
function buildSoccerBetBoard(allOdds, containerName) {
  ...
  allOdds.forEach((currentGame) => {
    if (
      ...
      bothTeamsScore = round(currentGame.both_teams_to_score);
      firstOver = round(currentGame.over15goals);
      secondOver = round(currentGame.over25goals);
      thirdOver = round(currentGame.over35goals);
      firstOverVal = 1.5;
      secondOverVal = 2.5;
      thirdOverVal = 3.5;
      startTime = formatDateAI(currentGame.date);
    ...
    if (
      ...
      scorePredict = currentGame.correctscore;
      bothTeamsScore = round(currentGame.both_teams_to_score);
      firstOver = round(round(currentGame.over15goals));
      secondOver = round(currentGame.over25goals);
      thirdOver = round(currentGame.over35goals);
      firstOverVal = 1.5;
      secondOverVal = 2.5;
